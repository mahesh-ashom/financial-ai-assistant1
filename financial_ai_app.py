    ...
    if not df.empty:
        ...
        with col1:
            ...
            if pd.notna(selected_data.get('ROE')):
                st.metric("ROE", f"{selected_data['ROE']:.1f}%")  # FIXED: No automatic percentage conversion
            if pd.notna(selected_data.get('ROA')):
                st.metric("ROA", f"{selected_data['ROA']:.1f}%")  # FIXED: No automatic percentage conversion
            if pd.notna(selected_data.get('Net Profit Margin')):
                st.metric("Net Profit Margin", f"{selected_data['Net Profit Margin']:.1f}%")  # FIXED: No automatic percentage conversion
        ...
        with col3:
            ...
            if pd.notna(selected_data.get('Gross Margin')):
                st.metric("Gross Margin", f"{selected_data['Gross Margin']:.1f}%")  # FIXED: No automatic percentage conversion
            if pd.notna(selected_data.get('Debt-to-Assets')):
                st.metric("Debt-to-Assets", f"{selected_data['Debt-to-Assets']:.1f}%")  # FIXED: No automatic percentage conversion
        ...
        if st.button("🤖 Generate AI Analysis", type="primary", key="company_analysis"):
            with st.spinner("Analyzing financial data..."):
                ...
                with col_a:
                    st.metric("AI Predicted ROE", f"{results['predicted_roe']:.1f}%")  # FIXED: Manual percentage format
    ...
    if st.button("🔍 ANALYZE CUSTOM DATA", type="primary", key="custom_analysis"):
        ...
        with st.spinner("🤖 AI is analyzing your data..."):
            ...
            with result_col1:
                ...
                st.metric("ROE", f"{roe_display:.1f}%", help="Return on Equity")  # FIXED: Manual percentage format
            ...
            with analysis_col1:
                ...
                strengths = []
                if roa > 8.0:  # FIXED: Compare against percentage values, not decimals
                    strengths.append(f"Strong ROA ({roa:.1f}%)")  # FIXED: Manual percentage format
                if net_profit_margin > 10.0:  # FIXED: Compare against percentage values
                    strengths.append(f"Good profitability ({net_profit_margin:.1f}%)")  # FIXED: Manual percentage format
            ...
            with analysis_col2:
                ...
                concerns = []
                if roa < 5.0:  # FIXED: Compare against percentage values
                    concerns.append(f"Low ROA ({roa:.1f}%) - improve asset efficiency")  # FIXED: Manual percentage format
                if net_profit_margin < 5.0:  # FIXED: Compare against percentage values
                    concerns.append(f"Low margins ({net_profit_margin:.1f}%) - reduce costs")  # FIXED: Manual percentage format
